...
<script type="text/javascript">
   ...
   $(function() {
	...
	<body id="body">
		...
		<div id="content">
		<% if(userId == "root") {%>
		<a href="shop_update.jsp?code=<%=code %>&price=<%=price %>&content=<%=content %>&pstock=<%=pstock %>" style="float: right;">상품수정</a><br>
		<%} else {%>
		...
		<%} %>
			...
			${vo.scomment} <input type="hidden" name="code" value=<%=code %>><input type="hidden" name="scomment" value=${vo.scomment}><input name="scpw" style="float: right;" value="비밀번호를 입력해주세요"><input type="submit" style="float: right;" value="삭제"><br><hr><br>
			</c:if>
</c:forEach>
</form>
<form action="ccreate">
